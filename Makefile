...
MODULE_LINK_LIBS=$(LINK_LIB_SWITCH)$(OFFICE_PROGRAM_PATH) $(LINK_LIB_SWITCH)$(PRJ)$(PS)lib
...
# LOADER
LOADER_MISC_FILES=$(LOADER_BUILD_DIR)/$(README_NAME) $(LOADER_BUILD_DIR)/$(LICENSE_NAME) \
	$(LOADER_BUILD_DIR)/$(CHANGES_NAME) $(LOADER_BUILD_DIR)/$(DESCRIPTION_XML_NAME) \
	$(LOADER_MANIFEST) 

$(LOADER_BUILD_DIR)/$(README_NAME) : $(README_NAME)
	$(COPY) "$(subst /,$(PS),$(README_NAME))" "$(subst /,$(PS),$(LOADER_BUILD_DIR))"

$(LOADER_BUILD_DIR)/$(LICENSE_NAME) : $(LICENSE_NAME)
	$(COPY) "$(subst /,$(PS),$(LICENSE_NAME))" "$(subst /,$(PS),$(LOADER_BUILD_DIR))"

$(LOADER_BUILD_DIR)/$(CHANGES_NAME) : $(CHANGES_NAME)
	$(COPY) "$(subst /,$(PS),$(CHANGES_NAME))" "$(subst /,$(PS),$(LOADER_BUILD_DIR))"

$(LOADER_BUILD_DIR)/$(DESCRIPTION_XML_NAME) : $(PYUNO_DIR)/description$(PY_MAJOR).xml
	$(COPY) "$(subst /,$(PS),$(PYUNO_DIR)/description$(PY_MAJOR).xml)" "$(subst /,$(PS),$(LOADER_BUILD_DIR)/$(DESCRIPTION_XML_NAME))"


$(LOADER_EXT_PKG) : $(BUILD_PYTHON_LIB) $(LOADER_BUILD_LIB_DIR) $(LOADER_REGISTRATION_COMPONENTS) \
...
$(LIB_PYUNO) : $(LIB_PYUNO_OBJ_FILES)
	$(LINK) $(LINK_OUT_FLAG)$(LIB_PYUNO) $(MODULE_LINK_FLAGS) $(LIB_PYUNO_OBJ_FILES) $(MODULE_LINK_LIBS) $(PYTHON_LINK_LIB) $(SALLIB) $(SALDYLIB) $(SALHELPERLIB) $(CPPULIB) $(CPPUHELPERLIB) $(STC++LIB) $(CPPUHELPERDYLIB) $(CPPUDYLIB) $(ADDITIONAL_LIBS) $(PYTHON_LIB) $(ADDITIONAL_LIBS)
...
$(LOADER_COMP) : $(LOADER_OBJ_FILES) $(LIB_PYUNO) $(REGISTERFLAG)
	$(LINK) $(LINK_OUT_FLAG)$(LOADER_COMP) $(COMP_LINK_FLAGS) $(LOADER_OBJ_FILES) $(MODULE_LINK_LIBS) $(LOADER_LINK_LIB) $(PYTHON_LINK_LIB) $(SALLIB) $(SALDYLIB) $(SALHELPERLIB) $(CPPULIB) $(CPPUHELPERLIB) $(STC++LIB) $(CPPUHELPERDYLIB) $(CPPUDYLIB) $(PYUNO_LIB) $(PYTHON_LIB) $(LOADER_COMP_ADDITIONAL_LIBS) $(ADDITIONAL_LIBS)

$(MODULE_OUT_SLO)/%.$(OBJ_EXT) : $(MODULE_DIR)/%.cxx $(MODULE_OUT_SLO) $(REGISTERFLAG)
	$(CC) $(CC_OUTPUT_SWITCH)$(subst /,$(PS),$@) $(CC_FLAGS) $< $(MODULE_CC_INCLUDES) $(CC_DEFINES) $(VERSION_DEF) 

$(MODULE_OUT_SLO)/%.$(OBJ_EXT) : $(MODULE_DIR)/%.c
	$(CC) $(CC_OUTPUT_SWITCH)$(subst /,$(PS),$@) $(CC_FLAGS) $< $(MODULE_CC_INCLUDES) $(CC_DEFINES) $(VERSION_DEF) 

$(MODULE_OUT_SLO)/%.$(OBJ_EXT) : $(LOADER_DIR)/%.cxx $(MODULE_OUT_SLO) $(REGISTERFLAG)
	$(CC) $(CC_OUTPUT_SWITCH)$(subst /,$(PS),$@) $(CC_FLAGS) $< $(LOADER_CC_INCLUDES) $(CC_DEFINES) $(VERSION_DEF) 

$(LOADER_PY_FILE) : 
	$(COPY) "$(subst /,$(PS),$(LOADER_DIR)/pythonloader.py)" "$(subst /,$(PS),$(LOADER_BUILD_LIB_DIR))"
...
$(MODULE_UNO_PY_FILE) : 
	$(COPY) "$(subst /,$(PS),$(MODULE_DIR)/$(MODULE_UNO_PY_NAME))" "$(subst /,$(PS),$(LOADER_BUILD_LIB_DIR))"

$(MODULE_HELPER_FILE) :
	$(COPY) "$(subst /,$(PS),$(MODULE_DIR)/$(MODULE_HELPER_PY_NAME))" "$(subst /,$(PS),$(LOADER_BUILD_LIB_DIR))"
...
$(SCRIPTING_PY) : $(SCRIPTING_PY_FILE)
	$(COPY) "$(subst /,$(PS),$(SCRIPTING_PY_FILE))" "$(subst /,$(PS),$(SCRIPTING_BUILD_DIR))"

$(SCRIPTING_MAILMERGE_PY) : $(SCRIPTING_MAILMERGE_PY_FILE)
	$(COPY) "$(subst /,$(PS),$(SCRIPTING_MAILMERGE_PY_FILE))" "$(subst /,$(PS),$(SCRIPTING_BUILD_DIR))"

$(SCRIPTING_BUILD_DIR)/$(README_NAME) : $(README_NAME)
	$(COPY) "$(subst /,$(PS),$(README_NAME))" "$(subst /,$(PS),$(SCRIPTING_BUILD_DIR))"

$(SCRIPTING_BUILD_DIR)/$(LICENSE_NAME) : $(LICENSE_NAME)
	$(COPY) "$(subst /,$(PS),$(LICENSE_NAME))" "$(subst /,$(PS),$(SCRIPTING_BUILD_DIR))"

$(SCRIPTING_BUILD_DIR)/$(CHANGES_NAME) : $(CHANGES_NAME)
	$(COPY) "$(subst /,$(PS),$(CHANGES_NAME))" "$(subst /,$(PS),$(SCRIPTING_BUILD_DIR))"

$(SCRIPTING_BUILD_DIR)/pythonscript.components : $(SCRIPTING_REGISTRATION_COMPONENTS)
	$(COPY) "$(subst /,$(PS),$(SCRIPTING_REGISTRATION_COMPONENTS))" "$(subst /,$(PS),$(SCRIPTING_BUILD_DIR))"

$(SCRIPTING_BUILD_DIR)/description.xml : $(SCRIPTING_DESCRIPTION_XML)
	$(COPY) "$(subst /,$(PS),$(SCRIPTING_DESCRIPTION_XML))" "$(subst /,$(PS),$(SCRIPTING_BUILD_DIR))"

$(SCRIPTING_BUILD_DIR)/META-INF/manifest.xml : $(SCRIPTING_MANIFEST)
	@-$(MKDIR) $(subst /,$(PS),$(SCRIPTING_BUILD_DIR)/META-INF)
	$(COPY) "$(subst /,$(PS),$(SCRIPTING_MANIFEST))" "$(subst /,$(PS),$(SCRIPTING_BUILD_DIR)/META-INF)"
